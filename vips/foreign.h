...
int
cgo_vips_jpegload(const char *filename, VipsImage **out)
{
    return vips_jpegload(filename, out, "access", VIPS_ACCESS_SEQUENTIAL, NULL);
}

int
cgo_vips_jpegload_shrink(const char *filename, VipsImage **out, int shrink)
{
